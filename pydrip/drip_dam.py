    ...
    def update_missing_data(self, american_rivers_df):
        ...
        if (
            ...
            if ar_id_data["Dam_Name"][0]:
                ...
                if (
                    "(" in ar_id_data["Dam_Name"][0]
                    and ")" in ar_id_data["Dam_Name"][0]
                ):
                ...
                current_dam_names = self.dam_alt_name + [
                    self.dam_name
                ]  # all dam names currently tracked in py object
                new_names = [
                    i
                    for i in ar_alt_dam_name
                    if i.lower() not in [x.lower() for x in current_dam_names]
                    and i != ""
                ]
                if len(new_names) > 0:
                    self.dam_alt_name + list(set(new_names))
                    if len(self.dam_alt_name) > 0 and len(self.dam_alt_name) > (
                        len(current_dam_names) - 1
                    ):  # For some reason it was adding dam_alt_name even if no alt name
